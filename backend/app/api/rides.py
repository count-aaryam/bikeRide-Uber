...
from app.schemas.ride import RideCreate, RideOut, RideOTPVerify
...
def accept_ride_endpoint(ride_id: int, current_user=Depends(get_current_user)):
    if current_user["role"] != "driver":
...
def driver_arriving_endpoint(ride_id: int, current_user=Depends(get_current_user)):
    if current_user["role"] != "driver":
        raise HTTPException(status_code=403, detail="Only drivers can update arrival status")
    ride = driver_arriving(ride_id=ride_id, driver_id=current_user["user_id"])
    return {
...
def start_ride_endpoint(ride_id: int, body: RideOTPVerify, current_user=Depends(get_current_user)):
    if current_user["role"] != "driver":
...
def complete_ride_endpoint(ride_id: int, current_user=Depends(get_current_user)):
    if current_user["role"] != "driver":
...
def cancel_ride_endpoint(ride_id: int, current_user=Depends(get_current_user)):
    ride = cancel_ride(
        ride_id=ride_id,
